      ...
      <div className="mx-auto max-w-6xl">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-8 shadow-2xl shadow-slate-950/30 backdrop-blur-xl sm:p-10">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-sm font-medium uppercase tracking-[0.3em] text-sky-300">Shopping list</p>
              <h1 className="mt-2 text-3xl font-semibold text-white">Everything you need, neatly arranged</h1>
            </div>
            <p className="text-sm text-slate-300">Browse your items in a calmer, more polished view.</p>
          </div>

          <div className="mt-8">
            {items.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-white/20 bg-slate-900/40 p-8 text-center">
                <p className="text-lg font-medium text-white">No items available yet.</p>
                <p className="mt-2 text-sm text-slate-400">Your shopping list will appear here as soon as items are added.</p>
              </div>
            ) : (
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {items.map((item) => (
                  <div key={item._id} className="overflow-hidden rounded-2xl border border-white/10 bg-slate-900/50 shadow-lg shadow-slate-950/20">
